...
% Output:
% 
% - obj : An object of class nb_cs where the datasets from the DB  
...
% Copyright (c) 2023, Kenneth Sæterhagen Paulsen
    ...
    if ~isa(obj,'nb_cs') || ~isa(DB,'nb_cs')
        error([mfilename ':: The two merged objects must both be of class nb_cs.'])
    end
    ...
    elseif isempty(DB)
        ...
        % Ensure the nb_cs object have the same types
        [obj, DB.data] = updateData(obj,DB.types,DB.data,'types');
        ...
        if isa(obj.data,'nb_distribution')
            tempData(obj.numberOfTypes, length(obj.variables), obj.numberOfDatasets) = nb_distribution;
        else
            tempData = nan(obj.numberOfTypes, length(obj.variables), obj.numberOfDatasets);
        end
