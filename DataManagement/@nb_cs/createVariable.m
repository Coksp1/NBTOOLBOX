...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    for ii = 1:length(expression)
        ...
        if inputs.overwrite
            ...
            vars = [vars,nameOfNewVariable{ii}]; %#ok<AGROW>
            try
                obj = obj.addVariable(obj.types, newData, nameOfNewVariable{ii});
            catch Err

                if ~ischar(nameOfNewVariable{ii})
                    error([mfilename ':: The input ''nameOfNewVariable'' must be a string or a cellstr.'])
                elseif strcmp(Err.message,['nb_cs :: The variable ''' nameOfNewVariable{ii} ''' already exists. Use setValue.'])
