function obj = addDataset(obj,dataset,NameOfDataset,startDate,variables)
% Syntax:
%
...
% Description:
%
% Makes it possible to add one dataset to a existing nb_ts
% object
% 
...
%                   vintage tag.
% 
% - startDate     : Must be a string or an object which is of a 
%                   subclass of nb_date with the start date of the 
%                   data. Have only effect (and then it must be 
%                   given) when you try to add data from a 
%                   numerical matrix and fetch data from FAME.
% 
% - variables     : A cell array of strings with the names of the
%                   variables you want to add. Only needed when a
%                   numerical matrix of data, a tseries object, a 
...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    [dataOfNewDataBase, variablesOfNewDataBase, startDateNewDataBase, endDateNewDataBase, freq, userdata,localVariables] = ...
        nb_ts.dataset2Properties(dataset,startDate,variables,NameOfDataset,obj.sorted);
    ...
    if size(dataOfNewDataBase,2) ~= size(variablesOfNewDataBase,2) 
        ...
        if any(count>1)
            ...
            error([mfilename ':: The data has not the same size (' int2str(size(dataOfNewDataBase,2)) ') as the number of unique variables ('...
                             int2str(size(variablesOfNewDataBase,2)) ') you are trying to add to the nb_ts object; ' NameOfDataset '(dataset nr. ' int2str(obj.numberOfDatasets + 1) ' of the object)'])
        end
    end
