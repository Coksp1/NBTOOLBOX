...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    if isempty(obj)
        string = {[varName ' = nb_ts();']};
    else
    
        % Get the code to initialize the nb_ts object
        %--------------------------------------------
        string = {};
        if isUpdateable(obj)
            ...
            for ii = 1:obj.numberOfDatasets
                ...
                if isempty(sources)
                    ...
                    string = [string;{[varName '_' int2str(ii) ' = nb_ts(''' sName '.mat'');']}];
            ...
            string = [string;{[varName ' = nb_ts(''' tempName '.mat'');']}];
...
% Get the code for initializing the sources as objects
    ...
    switch lower(subLinks.sourceType)
        ...
        case 'mat'
            ...
            source    = nb_pathCorrect(subLinks.source);
            string    = [string;{[varName '_' int2str(page) '_' int2str(sourceNum) ' = nb_ts(''' source ''');']}];
        ...
        case 'private' % Data without source
            ...
            variables = subLinks.variables;
            startDate = subLinks.startDate;
            temp      = nb_ts(source,'',startDate,variables);
            sName     = [tempName '_' int2str(page) '_'  int2str(sourceNum)];
            saveDataBase(temp,'ext','mat','saveName',sName);
            string    = [string;{[varName '_' int2str(page) '_' int2str(sourceNum) ' = nb_ts(''' sName '.mat'');']}];
