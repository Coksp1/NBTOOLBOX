...
% Description:
%
% A method for creating an nb_ts object with the estimated kernel density 
...
% Copyright (c) 2023, Kenneth Sæterhagen Paulsen
    ...
    if recursive
        ...
        for tt = indS:T
            ...
            for jj = 1:N
                dist(tt-indS+1,jj) = nb_distribution('type','kernel','parameters',{x(jj,:)',F(jj,:)'},'name',[obj.variables{jj} '_' toString(obj.startDate + (tt - 1))]);
            end
        end
        ...
        for jj = 1:N    
            dist(1,jj) = nb_distribution('type','kernel','parameters',{x(jj,:)',F(jj,:)'},'name',[obj.variables{jj} '_' toString(obj.endDate)]);
        end
        % Update properties object
        obj.data      = dist;
