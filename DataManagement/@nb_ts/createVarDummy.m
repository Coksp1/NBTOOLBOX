    ...
    for ll = 1:length(ind)
        ...
        for ii = 1:size(testsTemp,2)
            ...
            elseif isnumeric(tested) && isscalar(tested)
                ...
                error([mfilename ':: All tested inputs must be either a nb_ts object or a double (scalar).'])
            end
            dummyT = dummyT & logi;
...
function ind = testAgainstObject(condition,tested,varData)
    ...
    if size(tested,1) ~= size(varData,1)
        error([mfilename ':: When the a restriction is added as an nb_ts object. '...
