function [obj,m,s] = mZScore(obj,backward,forward,shift)
% Syntax:
%
...
%              Z-Score
%
% - m        : A double matrix with same size as obj storing the mean of 
%              the z-score calculations. 
% 
% - s        : A double matrix with same size as obj storing the std of 
%              the z-score calculations.
% Examples: 
% 
...
% Copyright (c) 2024, Kenneth Sæterhagen Paulsen
    ...
    for ii = 1 + backward + shift: size(d,1) - forward     
        ...
        s(ii,:,:) = std(d(ii - backward - shift:ii + forward - shift,:,:),0,1,'omitnan');
        n(ii,:,:) = (d(ii,:,:) - m(ii,:,:))./s(ii,:,:);
    end
    obj.data = n;
    ...
    if obj.isUpdateable() 
        ...
        % object
        obj = obj.addOperation(@mZScore,{backward,forward,shift});
    end
