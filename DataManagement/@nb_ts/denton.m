...
% Input:
% 
% - obj  : A nobs x nvars x npage nb_ts object with the main variables to 
%          transform.
%
...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    if isempty(z)
        ...
        if ~isa(z,'nb_ts')
            error([mfilename ':: The z input must be an object of class nb_ts.'])
        end
        if z.frequency ~= outFreq
            error([mfilename ':: The z input must have frequency ' nb_date.getFrequencyAsString(outFreq)])
        end
        if z.numberOfDatasets ~= obj.numberOfDatasets
        ...
        ret = hasVariable(zTemp,obj.variables);
        if any(~ret)
            error([mfilename ':: The following variables are no part of the z input: ' toString(obj.variables(~ret))])
        end
        zTemp = window(zTemp,'','',obj.variables);
        zTemp = reorder(zTemp,obj.variables);
        zData = double(zTemp);
    ...
    obj.endDate   = endZ;
    obj.data      = dentonData;
    obj.frequency = startZ.frequency;
    ...
    if obj.isUpdateable() && ~obj.isBeingMerged
        ...
        % object
        obj = obj.addOperation(@denton,{z,k,type,d});
