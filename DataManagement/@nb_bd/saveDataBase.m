...
%                             using the nb_ts.struct function.   
%
%                > 'txt'    : Saves the data down to a txt file.
%
%                > 'fame' ('db'): Writes a FAME database with the
%                             data stored in the nb_ts object.
%                             It is not possible to write to a 
%                             database stored in the data warehouse.
% 
...
% 
% - 'dateformat' : Sets the date format of the saved output Only if
%                  'ext' are set to 'xlsx', which is default.
...
%                  > 'NBEnglish' ('NBEngelsk')
%
%                  See the toDates methods of the classes nb_day,
...
% Copyright (c) 2024, Kenneth Sæterhagen Paulsen
    ...
    foundDateFormat = 'default'; % 0 : Date format 'yyyy', 'yyyyQq' 'yyyyMm(m)' or 'yyyyM(m)Dd(d)'. 1 : Date format 'dd.mm.yyyy' 
    foundSheets     =        {};
    ...
    switch lower(foundExt)
        
        case 'xlsx'
            
            datesOfData = dates(obj,foundDateFormat,'stripped');
            d           = getStrippedRep(obj);
            for ii = 1:obj.numberOfDatasets
                
                % Make a cell of the objects data, variables and
                % dates
                dataset = [{'Time'}, obj.variables; datesOfData, num2cell(d(:,:,1))];
                ...
                for ii = 1:obj.numberOfObservations
                    ...
                    for jj = 1:obj.numberOfVariables
                        ...
                        if isnan(d(ii,jj,kk))
                            ...
                            fprintf(writer,['\t' num2str(d(ii,jj,kk)) '\t']);
                        end
