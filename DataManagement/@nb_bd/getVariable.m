function variableData = getVariable(obj,type,variableName,startDate,endDate,pages)
% Syntax:
%
...
%                  given all dates where all observations of the data is 
%                  nan is removed, otherwise all dates are returned.
% 
...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    if ~any(strcmpi(type,{'full','stripped'}))
        pages        = endDate;
        endDate      = startDate;
        startDate    = variableName;
        variableName = type;
    end
    ...
    if isempty(obj)
        ...
        if strcmpi(type,'stripped')
            if obj.numberOfDatasets > 1
                dataT        = getFullRep(obj);
                variableData = dataT(startInd:endInd,:,:);
                isNaN        = all(all(isnan(variableData),2),3);
                variableData = variableData(~isNaN,varInd,pages);
            else
                [indR,indC]        = find(obj.locations);
                indR               = indR(indR >= startInd & indR <= endInd);
                rows               = unique(indR);
                numRows            = size(rows,1);
                variableData       = nan(numRows,1);
                indVar             = indC == find(varInd,1);
                indD               = ismember(rows,indR(indVar));
                variableData(indD) = obj.data(indVar);
            end
        else
            dataT        = getFullRep(obj);
            variableData = dataT(startInd:endInd,varInd,pages);
        end
