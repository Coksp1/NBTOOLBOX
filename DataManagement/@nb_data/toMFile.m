...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    if isempty(obj)
        string = {[varName ' = nb_data();']};
    else
    
        % Get the code to initialize the nb_data object
        %--------------------------------------------
        string = {};
        if isUpdateable(obj)

            % Write code to construct nb_ts objects of all sources
            string = [string;{[varName ' = nb_ts();']}];
            for ii = 1:obj.numberOfDatasets
                ...
                if isempty(sources)
                    ...
                    string = [string;{[varName '_' int2str(ii) ' = nb_data(''' sName '.mat'');']}];
            ...
            string = [string;{[varName ' = nb_data(''' tempName '.mat'');']}];
...
% Get the code for initializing the sources as objects
    ...
    switch lower(subLinks.sourceType)
        ...
        case 'mat'
            ...
            source    = nb_pathCorrect(subLinks.source);
            string    = [string;{[varName '_' int2str(page) '_' int2str(sourceNum) ' = nb_data(''' source ''');']}];
            
        case 'private' % Data without source
            
            source    = subLinks.source;
            variables = subLinks.variables;
            startObs  = subLinks.startDate;
            temp      = nb_data(source,'',startObs,variables);
            sName     = [tempName '_' int2str(page) '_'  int2str(sourceNum)];
            saveDataBase(temp,'ext','mat','saveName',sName);
            string    = [string;{[varName '_' int2str(page) '_' int2str(sourceNum) ' = nb_data(''' sName '.mat'');']}];
