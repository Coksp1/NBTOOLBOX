function xout = nb_nanmstd(xin,backward,forward,flag)
% Syntax:
% 
% xout = nb_nanmstd(xin,backward,forward,flag)
% 
% Description:
% 
...
% Examples:
% 
% newseries = nb_nanmstd(x,9,0); % 10-element moving standard deviation
% 
...
% Copyright (c) 2024, Kenneth Sæterhagen Paulsen
    ...
    if ~flag
        for ii = 1:min(backward,size(xin,1) - forward)   
            xout(ii,:,:) = std(xin(1:ii + forward,:,:),0,'omitnan');
        end
    end
    if size(xin,1) > backward
        for ii = 1 + backward: size(xin,1) - forward     
            xout(ii,:,:) = std(xin(ii - backward:ii + forward,:,:),0,'omitnan');
        end 
    end
    if ~flag
        for ii = size(xin,1) - forward + 1:size(xin,1)   
            xout(ii,:,:) = std(xin(ii:end,:,:),0,'omitnan');
        end
    end
