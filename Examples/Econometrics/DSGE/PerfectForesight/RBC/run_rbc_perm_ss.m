...
m = nb_dsge('nb_file','rbc_stoch3.nb','silent',false,'optimset',opt);
...
m  = checkSteadyState(m,'steady_state_file','rbc_steadystate3');
ss = m.getSteadyState();
...
%% Solve for a permanent shock to productivity
% Using block decomposition to solve the perfect foresight problem

periods      = 80;
e_a_val      = 0.1;
endVal       = getEndVal(m,'steady_state_exo',struct('e_a',e_a_val));
exoVal       = nb_data(ones(periods,1)*e_a_val,'',1,{'e_a'});
[sim,pBlock] = perfectForesight(m,'endVal',endVal,'exoVal',exoVal,...
                                  'periods',periods,'blockDecompose',true);
% nb_graphSubPlotGUI(pBlock);

%% Plot together

p = merge(pFirst,pBlock);
p.set('legends',{'Normal','Block'});
