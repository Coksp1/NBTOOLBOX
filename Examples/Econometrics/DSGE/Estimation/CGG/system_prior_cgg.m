...
% System priors
%---------------
yAutoPrior   = @(x)log(nb_distribution.normal_pdf(x,0.0063,0.05));
pieAutoPrior = @(x)log(nb_distribution.normal_pdf(x,0.0001,0.05));
systemPrior  = {'y','y',1,yAutoPrior;
                'pie','pie',1,pieAutoPrior};
ms           = setSystemPrior(m,'cov',systemPrior);
% yCorrPrior   = @(x)log(nb_distribution.normal_pdf(x,0.8,0.05));
% systemPrior2 = {'y','pie',1,yCorrPrior};
% ms           = setSystemPrior(m,'cov',systemPrior,'corr',systemPrior2);
% yFEVDPrior  = @(x)log(nb_distribution.normal_pdf(x,0.2,0.02));
% yFEVDPrior2 = @(x)log(nb_distribution.normal_pdf(x,0.4,0.02));
% systemPrior = {'eps','y',  1,yFEVDPrior;
%                'e',  'pie',2,yFEVDPrior2};
% ms          = setSystemPrior(m,'fevd',systemPrior);
% yIRFPrior  = @(x)log(nb_distribution.normal_pdf(x,1,0.2));
% yIRFPrior2 = @(x)log(nb_distribution.normal_pdf(x,-1,0.2));
% systemPrior = {'eps','y',  1,yIRFPrior;...
%                'e',  'pie',2,yIRFPrior2};
% ms         = setSystemPrior(m,'irf',systemPrior);
