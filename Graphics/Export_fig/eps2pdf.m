function eps2pdf(source, dest, crop, append, gray, quality, flip, gs_options)
...
%   eps2pdf(source, dest, crop, append, gray, quality, flip, gs_options)
%
...
%   flip    - true or false. Gice true to flip the PDF
...
% 18/11/15: Added the flip input, Kenneth S. Paulsen,
...
% Copyright (c) 2024, Kenneth Sæterhagen Paulsen
    ...
    % Rotate the figure, edited by Kenneth S. Paulsen
    %==============================================================
    if nargin < 7
        flip = false;
    end
    if flip
        options = [options ' -c "<</Orientation 3>> setpagedevice"'];
    end
    %==============================================================
    
    % Enable users to specify optional ghostscript options (issue #36)
    if nargin > 7 && ~isempty(gs_options)
    ...
    if status
        ...
        [pd,fd,ed]   = fileparts(dest);
        if strcmpi(p,pd)
            oldPath = cd(p);
            options = regexprep(options, '-f ".+"',['-f "',f,e,'"']);
            options = regexprep(options, '-sOutputFile=[^ ]+',['-sOutputFile="',fd,ed '"']);
            cdBack  = true;
        else
            cdBack = false;
        end
        ...
        elseif ~isempty(strfind(message,'/typecheck in /findfont'))
            ...
            message   = sprintf(['Ghostscript error: could not find the following font(s): ' font_name '\n']);
            fpath     = fileparts(mfilename('fullpath'));
            gs_fonts_file = fullfile(fpath, '.ignore', 'gs_font_path.txt');
            message   = [message, sprintf(['Try to add the font''s folder to your ' strrep(gs_fonts_file,'\','\\') ' file\n\n'])];
            error(['export_fig error:: ', message]);
        else
            message = strrep(message,'\','\\');
            message = [message,sprintf(['\nGhostscript error: perhaps '  strrep(dest,'\','\\') ' is open by another application\n'])];
            if ~isempty(gs_options)
                message = [message, sprintf(['  or maybe the ' gs_options ' option(s) are not accepted by your GS version\n'])];
            end
            message = [message,  sprintf('  or maybe you have another gs executable in your system''s path\n')];
            message = [message,  sprintf(['Ghostscript options: ' strrep(orig_options,'\','\\')])];
            error(message);
        end
    end
end
