...
%
% Edited by Kenneth SÊterhagen Paulsen
    ...
    try
        ...
        if isvector(options)
            ...
            try
                ...
                % Generate a pdf
                eps2pdf(tmp_nam, pdf_nam_tmp, 1, options.append, options.colourspace==2, options.quality, options.flip, options.gs_options);
            ...
            if options.eps || options.linecaps
                try
                    ...
                    if options.linecaps
                        ...
                        if options.pdf
                            eps2pdf(eps_nam_tmp, pdf_nam, 1, options.append, options.colourspace==2, options.quality, options.flip, options.gs_options);
                        end
                    end
        ...
        if displaySuggestedWorkarounds && ~strcmpi(err.message,'export_fig error') 
            ...
            message = [message, sprintf('  that you are using the <a href="https://github.com/altmany/export_fig/archive/master.zip">latest version</a> of export_fig\n')];
            if ismac
                message = [message, sprintf('  and that you have <a href="http://pages.uoregon.edu/koch">Ghostscript</a> installed\n')];
            else
                message = [message, sprintf('  and that you have <a href="http://www.ghostscript.com">Ghostscript</a> installed\n')];
            end
            try
                if options.eps
                    message = [message, sprintf('  and that you have <a href="http://xpdfreader.com/download.html">pdftops</a> installed\n')];
                end
            catch
                % ignore - probably an error in parse_args
            end
            message = [message, sprintf('  and that you do not have <a href="matlab:which export_fig -all">multiple versions</a> of export_fig installed by mistake\n')];
            message = [message, sprintf('  and that you did not made a mistake in the <a href="matlab:help export_fig">expected input arguments</a>\n')];
            try
                % Alert per issue #149
                if ~strncmpi(get(0,'Units'),'pixel',5)
                    message = [message, sprintf('  or try to set groot''s Units property back to its default value of ''pixels'' (<a href="matlab:web(''https://github.com/altmany/export_fig/issues/149'',''-browser'');">details</a>)\n')];
                end
            catch
