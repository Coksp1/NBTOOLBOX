classdef nb_donut < nb_plotHandle & nb_notifiesMouseOverObject
% Syntax:
%     
...
%     
% This class will make the donut chart automatically fit the axes of 
% the plot.
%  
...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    properties
        ...
        % A logical array with ones for the slice to bite. I.e. [0,0,1,0]; 
        ...
        % names. Must be of size 1 x nSlices. See the method 
        ...
        % Force there to be no labels of the pies. If yData gives more
        % donuts noLabels will be foreced to 1 (true).
        noLabels        = 0;      
    ...
    methods
        ...
        function set.innerRadius(obj,value)
            if ~nb_isScalarNumber(value)
                error([mfilename ':: The innerRadius property must be given'...
                    ' as a scalar double.'])
            end
        ...
        function set.radius(obj,value)
            if ~nb_isScalarNumber(value)
                error([mfilename ':: The radius property must be given'...
                    ' as a scalar double.'])
            end
            obj.radius = value;
        end
...
%         end         
        ...
        function obj = nb_donut(yData,varargin)
        ...
        function [startAngle, endAngle] = getSliceAngle(obj, index1,index)
            pieData    = obj.yData(index1,:) / sum(obj.yData(index1,:));
            startAngle = pi/2 + sum(pieData(1:index-1)) * 2*pi;
            endAngle   = startAngle + pieData(index) * 2*pi;
        end
        
        function [x, y, angle] = getSlicePosition(obj, index1, index, relativeAngle, relativeRadius)
            ...
            [startAngle,endAngle] = obj.getSliceAngle(index1, index);
            angle                 = startAngle + relativeAngle .* (endAngle - startAngle);
            angle                 = mod(angle, 2 * pi);
            [x, y]                = pol2cart(angle, obj.radius + relativeRadius);
    ...
    methods (Access=protected)
        ...
        function plotDonut(obj)
            ...
            relRadius = linspace(obj.innerRadius,0,size(data,1)+1);
            if size(data,1) > 1
                obj.noLabels = true;
            end
            ...
            try
                ...
                for jj = 1:size(data,1)
                    ...
                    for ii = 1:size(data,2)
                        ...
                        [xx, yy]       = obj.getSlicePosition(jj, ii, relativeAngles, relRadius(jj+1));
                        [xxInn, yyInn] = obj.getSlicePosition(jj, ii, relativeAngles, relRadius(jj));
                        xx             = [xx, fliplr(xxInn)]; %#ok<AGROW>
                        yy             = [yy, fliplr(yyInn)]; %#ok<AGROW>
                        ...
                        if ~obj.noLabels
                            ...
                            labelDistance           = 0.2;
                            [xLabel, yLabel, angle] = obj.getSlicePosition(jj, ii, 0.5, labelDistance);
                            if angle < pi/4
                                hAlignment = 'left';
                                vAlignment = 'middle';
                            elseif angle < pi*3/4
                ...
                elseif isnumeric(obj.cData)
                    ...
                    if size(obj.cData,1) ~= size(obj.yData,2) || size(obj.cData,2) ~= 3
    ...
    methods(Hidden=true)
        ...
        % point : current point in data units
            ...
            [angle,radius]     = cart2pol(point(1), point(2));
            angle(angle < pi/2) = angle(angle < pi/2) + 2*pi;
            
            % Find the matching row
            donutRadius = 1 + linspace(obj.innerRadius,0,size(obj.yData,1)+1);
            x            = find(radius > donutRadius, 1, 'last');
            if isempty(x) || x == size(obj.yData,1)+1 
                x     = [];
                y     = [];
                value = [];
                return;
            ...
            pieData             = obj.yData(x,:) / sum(obj.yData(x,:));
