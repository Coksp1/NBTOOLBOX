...
% Copyright (c) 2023, Kenneth Sæterhagen Paulsen
    ...
    if forLegend
        ...
        if strcmpi(plotType,'scatter')
            vars = [obj.scatterDates(1:2:end), obj.scatterDatesRight(1:2:end)];
        ...
        if ~isempty(obj.datesToPlot)
            vars = RemoveDuplicates(sort(obj.variablesToPlot));
            return
        end
        ...
        plotType = obj.plotType;
        if strcmpi(plotType,'scatter')
            vars = [nb_nestedCell2Cell(obj.scatterVariables), nb_nestedCell2Cell(obj.scatterVariablesRight)];
        elseif strcmpi(plotType,'candle')
