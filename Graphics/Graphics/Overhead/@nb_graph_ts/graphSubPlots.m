...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    %==============================================================
    if ~nb_isempty(obj.subPlotsOptions)
        yLabel     = obj.yLabel;
        yLim       = obj.yLim;
    ...
    for ii = 1:obj.numberOfGraphs
        ...
        for jj = 1:numberOfIteration
            ...
            %----------------------------------------------------------
            % Here we correct for missing observations
            %----------------------------------------------------------
            if ~isempty(obj.dataToGraph)
                % Here we update the properties 'dates' and 'xTick' (only if we strip the data)
                [obj,obj.dataToGraph] = interpretMissingData(obj,obj.dataToGraph,1); 
            end
            ...
            if iscell(obj.position) && nb_sizeEqual(obj.position,[1,numberOfPlotsPerSubPlot])
                obj.axesHandle = nb_axes('parent',obj.figureHandle(end),'position',obj.position{jj},'fast',fast);
            else
                if obj.subPlotSpecial
                    obj.axesHandle = nb_subplotSpecial(obj.subPlotSize(1),obj.subPlotSize(2),jj,'parent',obj.figureHandle(end),'fast',fast);
                else
                    obj.axesHandle = nb_subplot(obj.subPlotSize(1),obj.subPlotSize(2),jj,'parent',obj.figureHandle(end),'fast',fast);
                end
            end
            obj.axesHandle.update = 'off';

            % Subplot spesific options
            %------------------------------------------------------
            if ~nb_isempty(obj.subPlotsOptions)
                obj.yLabel     = yLabel;
                obj.yLim       = yLim;
                obj.ySpacing   = ySpacing;
                obj.dashedLine = dashedLine;
                var            = strrep(obj.variablesToPlot{(ii - 1)*numberOfPlotsPerSubPlot + jj},'.','_');
                if isfield(obj.subPlotsOptions,var)
                    if isfield(obj.subPlotsOptions.(var),'yLim')
                        obj.yLim = obj.subPlotsOptions.(var).yLim;
                    end
                    if isfield(obj.subPlotsOptions.(var),'ySpacing')
                        obj.ySpacing = obj.subPlotsOptions.(var).ySpacing;
                    end
                    if isfield(obj.subPlotsOptions.(var),'dashedLine')
                        obj.dashedLine = obj.subPlotsOptions.(var).dashedLine;
                    end
                    if isfield(obj.subPlotsOptions.(var),'yLabel')
                        obj.yLabel = obj.subPlotsOptions.(var).yLabel;
                    end
                end
            end
            ...
            % property 'DB'.
            %------------------------------------------------------
            if ~isempty(obj.fanDatasets) || ~isempty(obj.defaultFans)
