function graphEng(obj,figureHandle,gui)
% Syntax:
% 
% graphEng(obj)
% 
...
% Copyright (c) 2023, Kenneth Sæterhagen Paulsen
    ...
    if isa(obj.plotter,'nb_graph')
        legendsTemp = obj.plotter(1).legends;
        legAutoTemp = obj.plotter.legAuto;
    end
    ...
    for ii = 1:size(obj.plotter,2)
        obj.plotter(ii).language  = 'english';
    ...
    if isa(obj.plotter(1),'nb_graph')
        ...
        % If the legendsEng property is empty, we use the legends 
        % provided by the nb_graph object. This makes it possible to 
        % use the lookUpMatrix instead.
        if ~isempty(obj.legendsEng)
            obj.plotter(1).legends = obj.legendsEng;
            if size(obj.plotter,2) > 1
                obj.plotter(2).legends = obj.legendsEng;
            end
        end
    ...
    for ii = 1:size(obj.plotter,2)
        ...
        if isa(obj.plotter,'nb_graph')
            obj.plotter(ii).legends                = legendsTemp;
