function options = modelSelectionAlgorithm(options,minLags)
% Syntax:
%
...
% Copyright (c) 2023, Kenneth Sæterhagen Paulsen
    ...
    switch lower(options.modelSelection)
        ...
        case 'laglength'
            ...
            % The returend nLags is allways a scalar              
            if nLags == -1  
                % All the non fixed exogenous variables are
                % removed
                options.exogenous = options.exogenous(fixed);
            else
                % Add lag postfix (If the variable \already have a lag 
                % postfix we append the number indicating that it is 
                % lag once more)
        ...
        case 'autometrics'
            ...
            keep               = ~cellfun(@isempty,results{1}.nlags);
            exoNames           = exoNames(keep);
            options.nLags      = results{1}.nlags(keep); 
            newNames           = nb_appendLagPostfix(exoNames,options.nLags);
