...
%               nb_nlsEstimator.estimate function.
%
...
function res = resursivePrint(results,options,precision)
    ...
    res = sprintf('Method: %s',['Non-linear Least Squares' extra]);
    res = char(res,sprintf('STD: %s','Inverse Fischer information'));
    ...
    [exo,numExo] = nb_nlsEstimator.getCoeff(options);
...
function res = normalPrint(results,options,precision)
    ...
    res = sprintf('Method: %s','Non-linear Least Squares');
    res = char(res,sprintf('STD: %s','Inverse Fischer information'));
    res = char(res,sprintf('%s',nb_clock('gui')));
    res = char(res,sprintf('Elapsed time: %s',num2str(results.elapsedTime)));
    ...
    [coeff,numC] = nb_nlsEstimator.getCoeff(options);

    % Fill table
    table{1,1}           = 'Variable';
    table(1,2:end)       = options.parser.dependent;
    table(2:4:end,1)     = coeff;
    table(3:4:end,1)     = repmat({'(Std. Error)'},numC,1);
    table(4:4:end,1)     = repmat({'(t-Statistics)'},numC,1);
    table(5:4:end,1)     = repmat({'(P-Value)'},numC,1);
    ...
    if ~options.doTests
        ...
        tests = nb_addStars(tests,9,[],7,8,round(options.nLagsTests));
