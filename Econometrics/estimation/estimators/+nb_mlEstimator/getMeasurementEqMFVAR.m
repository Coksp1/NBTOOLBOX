function [H,freqD,extraAdded] = getMeasurementEqMFVAR(options,extra)
% Syntax:
%
...
% mapping (but still time-varying if we are dealing with weekly data)
    ...
    % Get the measurement equation for each sub-period
    H     = cell(nBreaks+1,1);
    opt   = options;
    for ii = 1:nBreaks+1
...
% into sub-periods due to changes in the frequency of some variables.
    ...
    if maxFreq == 52
        ...
        % In this case we get a time-varying measurement equation
        ...
        % Get measurement equation at each period in time
...
% Get one unique measurement equation
...
% Get measurement equation at each period in time from all the unique
% possible measurement equations already calculated.
    ...
    if size(H,2) <= 3
        ...
        while endWIter < endW 
            ...
            % Get the measurement equation at the low frequency measurement
            % points
            if ii < periods && ii > 0
...
function [HH,extraAdded] = doMixing(options,H,extraAdded,maxFreq,type,extra)
    ...
    % Adjust options struct to return only the measurement equations of the
