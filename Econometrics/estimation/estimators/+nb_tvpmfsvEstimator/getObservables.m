...
% Syntax:
%
% observables = nb_tvpmfsvEstimator.getObservables(results,options)
%
% Description:
%
...
% Copyright (c) 2023, Kenneth Sæterhagen Paulsen
    ...
    if ~isfield(results,'Z')
        ...
        if isfield(results.smoothed,'observables')
            observables = nb_ts(results.smoothed.observables.data, 'Observables',...
                results.smoothed.observables.startDate, ...
                results.smoothed.observables.variables,false);
        else
            ...
            for tt = 1:size(X,2)
                for vv = 1:size(X,1)
                    if isnan(X(vv,tt))
                        X(vv,tt) = results.smoothed.lambda.data(vv,:,tt)*alpha(:,tt);
                        if ~isempty(results.S)
                            X(vv,tt) = X(vv,tt)*results.S(vv,:,end);
                        end
