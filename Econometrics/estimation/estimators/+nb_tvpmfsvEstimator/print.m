...
% Syntax:
%
% res = nb_tvpmfsvEstimator.print(results,precision)
%
% Description:
%
...
function res = resursivePrint(results,options,precision)
    ...
    res = sprintf('Method: %s',['Bayesian' extra]);
    res = char(res,sprintf('Model: %s','Dynamic factor model (Schroder and Eraslan (2021))'));
    ...
    % Print   
    [coeff,nCoeff] = nb_tvpmfsvEstimator.getCoeff(options);
