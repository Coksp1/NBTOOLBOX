...
% Copyright (c) 2023, Kenneth Sæterhagen Paulsen
    ...
    % Print results
    %--------------------------------------------------------------
    res = sprintf('Test: %s','Autocorrelation of residual test');
    res = char(res,sprintf('Null hypothesis: %s','Residuals are not autocorrelated'));
    res = char(res,sprintf('%s',nb_clock('gui')));
    res = char(res,'');
    
    % Get the results
    autocorrTest = obj.results.autocorrTest;
    autocorrProb = obj.results.autocorrProb;
    numTest      = size(autocorrTest,2);
    
    % Print the F-test results
    table          = repmat({' '},[1 + numTest,3]);
    table{1,2}     = 'Test statistics';
    table{1,3}     = 'P-value';
    table(2:end,2) = nb_double2cell(autocorrTest,precision);
    table(2:end,3) = nb_double2cell(autocorrProb,precision);
