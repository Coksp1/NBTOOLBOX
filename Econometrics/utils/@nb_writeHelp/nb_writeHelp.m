...
% Copyright (c) 2024, Kenneth Sæterhagen Paulsen
    ...
    methods
        ...
        function obj = nb_writeHelp(name,option,typeName,dataType)
            ...
            obj.option     = option;
            ind            = strfind(name,'.');
            if ~strcmpi(typeName,{'class','package'})
                templateFunc = str2func(name); 
                if isempty(typeName)
                    obj.options = fieldnames(templateFunc());
                else
                    obj.options = fieldnames(templateFunc(typeName));
                end
            else
                if ~isempty(ind)
    ...
    methods (Access=protected)
        ...
        function obj = a_bar_1(obj)
            
            obj.helpStruct.a_bar_1 = ...
                'Hyperparamter on own lags. Default is 0.5.';

        end

        function obj = a_bar_2(obj)
            
            obj.helpStruct.a_bar_2 = ...
                'Hyperparamter on other lags. Default is 0.5.';

        end

        function obj = a_bar_3(obj)
            
            obj.helpStruct.a_bar_3 = ...
                'Hyperparamter on exogenous variables. Default is 100.';

        end

        function obj = a_bar_ar(obj)
            
            obj.helpStruct.a_bar_ar = ...
                ['Prior standard deviation of AR terms. Default is 0.5. The standard deviation of lags are ',...
                 'divided by their lag number! I.e. with this parameter set to 0.5, lag 2 will get a ',...
                 'standard deviation of 0.25! A normal prior is assumed!'];

        end

        function obj = a_bar_exo(obj)
            
            obj.helpStruct.a_bar_exo = ...
                ['Prior standard deviation of exogenous and seasonal terms. Default is 100. A normal ',...
                 'prior is assumed!'];

        end

        function obj = a_bar_ma(obj)
            
            obj.helpStruct.a_bar_ma = ...
                ['Prior standard deviation of MA terms. Default is 0.5. The standard deviation of lags are ',...
                 'divided by their lag number! I.e. with this parameter set to 0.5, lag 2 will get a ',...
                 'standard deviation of 0.25! A normal prior is assumed!'];

        end

        function obj = a_bar_sigma(obj)
            
            obj.helpStruct.a_bar_sigma = ...
                ['Prior standard deviation of residual variance. Default is sqrt(10). A gamma prior is ',...
                 'assumed!'];
        ...
        function obj = algorithm(obj)
            ...
            if strcmpi(obj.nameOfPOrC,'nb_arima') || strcmpi(obj.nameOfPOrC,'nb_model_selection_group') || strcmpi(obj.nameOfPOrC,'nb_arimaEstimator')
                obj.helpStruct.algorithm = ...
                    ['Sets the method for ARIMA estimation, can be ''bayesian'', ''hr'' or ''ml'', ',...
                     'for Bayesian (nb_barima), Hannan-Rissan (nb_hrarima) and Maximum likelihood ',...
                     '(nb_mlarima) respectivly (see respective function for more details).'];
        ...
        function obj = arcoeff(obj)
            
            if strcmpi(obj.nameOfPOrC,'nb_arima.priorTemplate')
                obj.helpStruct.arcoeff = ...
                    'Prior on the first AR coefficient. Set to [] to use OLS estimate! [] is default.';
            else
                obj.helpStruct.arcoeff = ...
                    ['Hyperparameter on first lag coefficient of each equation. Default is 1. Set to nan to use the ',...
                    'estimate of AR(1) on each individual model as the prior.'];
            end

        end

        function obj = arcoeffhyperprior(obj)
            
            obj.helpStruct.arcoeffhyperprior = ...
                ['Hyperprior for the ''ArCoeff'' prior. A function handle, that takes one input, and that ',...
                'input is a scalar double with the value of ''ARCoeff''.'];
        ...
        function obj = burn(obj)
            obj.helpStruct.burn = ...
                'How many draws that should be used as burn in. Default is 500.';
        end
        ...
        function obj = coeff(obj)
            obj.helpStruct.coeff = ...
                ['A N x 2 cell array with specific priors on some coefficients. In the first column you ',...
                 'must provide the name of the coefficient, name of dependent + _ + name of rhs variable ',...
                 '+ lag specifier. E.g. ''Var1_Var1_lag1'' or ''Var1_Var2''. In the last example Var2 is an ',...
                 'exogenous variable. In the second column you give the prior value, as a scalar double.'];
        end

        function obj = combinatormodel(obj) 
            obj.helpStruct.combinatormodel = ...
                ['The model template for the final model that combines the indicator with the variable of interest ',...
        ...
        function obj = constantdiffuse(obj)
            obj.helpStruct.constantdiffuse = ...
                ['If this option is set to true, a diffuse prior for the ',...
                 'constant term is used instead of a Laplace prior. This means ',...
                 'that the posterior mode, mean and median of the constant term ',...
                 'will be the OLS estimate, instead of potentially being pushed ',...
                 'against zero.'];
        end
        ...
        function obj = covidadj(obj)
            obj.helpStruct.covidadj = ...
                ['Either a cellstr with dates or a vector of nb_date objects. Must be dates at ',...
                'the frequency of the dependent variable. All observations of the dependent ',...
                'variable at these dates are removed before estimation. The same applies to all ',...
                'regressors at the same "row". This is done so that these dates does not affect ',...
                'the estimated parameters of the model.'];
            if strcmpi(obj.nameOfPOrC,'nb_var')
                obj.helpStruct.covidadj = [obj.helpStruct.covidadj,...
                    ' Is not yet supported for when ''estim_method'' is set to ''ml'' or the prior is set to ',...
                    'one of the priors supporting mixed frequency data or missing observations, i.e. the ',...
                    'priors ending with ''MF'' or the prior ''kkse''.'];
            end
        end
        ...
        function obj = datesvd(obj)
            obj.helpStruct.datesvd = ...
                ['A one line char for where to start the stochastic-volatility-dummy prior. E.g. ',...
                 '''2020Q1''. This must be set, if SVD == true!'];
        end    

        function obj = delta(obj)
            obj.helpStruct.delta = ...
                ['Shrinkage parameter of the dummy-initial-observation prior. As delta goes to 0 all the ',...
                 'variables of the VAR are forced to be at their unconditional mean, or the system is ',...
                 'characterized by the presence of an unspecified number of unit roots without drift. ',...
                 'As such, the dummy-initial observation prior is consistent with cointegration'];
        end

        function obj = deltahyperprior(obj)
            obj.helpStruct.deltahyperprior = ...
                ['Hyperprior for the ''delta'' prior. A function handle, that takes one input, and that ',...
                'input is a scalar double with the value of ''delta''.'];
        end
        ...
        function obj = dio(obj)
            obj.helpStruct.dio = ...
                'Apply dummy-initial-observation prior by Sims (1993). true or false.';
        end
            ...
            elseif strcmpi(obj.nameOfPOrC,'nb_var') || strcmpi(obj.nameOfPOrC,'nb_bVarEstimator')
                obj.helpStruct.draws = ...
                    ['A integer with the number of draws of Monte Carlo integration or Gibbs sampler when doing '...
                     'bayesian estimation.'];
            elseif strcmpi(obj.nameOfPOrC,'nb_laplacePrior') 
                obj.helpStruct.draws = ...
                    ['Number of draws to simulate from the posterior distribution. ',...
                     'If set to 1, only one draw from the posterior is returned.'];
            else
        ...
        function obj = estim_method(obj)
            ...
            switch lower(obj.nameOfPOrC)
                ...
                case 'nb_sa'
                    extra = [' Supported are ordinary least squares (''ols'') and least absolute shrinkage and ',...
                        'selection operator (''lasso''), see nb_olsEstimator.estimate and nb_lassoEstimator.estimate.'];
                ...
                case 'nb_fmdyn'
                    extra = [' Expected maximum likelihood (''dfmeml'') or estimation method of Schröder and Eraslan (2021) ',...
                             '(''tvpmfsv''). See doc of the nb_fmdyn for more on these algorithms. Priors for the bayesian ',...
                             'methods can be set by using nb_fmdyn.priorTemplate and nb_fmdyn.setPrior methods.'];
                case 'nb_mfvar'
                    extra = [' This options should not be set. It will be ''ml'' (maximum likelihood) if the prior option ',...
                             'is empty, otherwise it will be set to ''bVar'' (bayesian VAR) or (''tvpmfsv''). See ',...
                             'nb_mlEstimator.estimate, nb_bVarEstimator.estimate or nb_tvpmfsvEstimator respectively. Priors ',...
                             'for the bayesian methods can be set by using nb_mfvar.priorTemplate and nb_mfvar.setPrior methods.'];
                case 'nb_midas'
                ...
                case 'nb_var'
                    extra = [' The supported options are; ordinary least squares (''ols'') , bayesian VAR estimation (''bVar'') ',...
                             ...
                             ', ridge regression (''ridge'') or estimation method of Schröder and Eraslan (2021) (''tvpmfsv''). If ',...
                             'the prior option is set, this option will automatically be set to ''bVar'' or ''tvpmfsv''. Priors ',...
                             'for the bayesian methods can be set by using nb_var.priorTemplate and nb_var.setPrior methods.'];
        ...
        function obj = f0varscale(obj)
            obj.helpStruct.f0varscale = ...
                ['Scale factor on the variance of the prior on the initial value of factors. ',...
                'Default is 10. N(0,f0VarScale*I).'];
            if any(strcmpi(obj.nameOfPOrC,{'nb_var.priorTemplate'}))
                obj.helpStruct.f0varscale = [obj.helpStruct.f0varscale, ' Do not adjust!'];
            end
        end
        ...
        function obj = folds(obj)
            obj.helpStruct.folds = ... 
                ['A cell with the folds (periods) for the cross-validation. Either (1) a '...
                'k x 2 cell where the first column contain start dates for the folds and the second column'...
                'contain end dates for the folds. (2) A 1 x 3 cellstr which contain k in the first element, '...
                'the start of the evaluation period in the second element, and the end of the evaluation '... 
                'period in the third element. The k folds will be equally sized over the evaluation period, '...
                '(not counting the remainder). All dates may be given as chars or nb_date objects.'];
        end
        
        function obj = foldweights(obj)
            obj.helpStruct.foldweights = ...
                'A 1 x k vector with the weights used when taking a weighted sum of the score criterion across folds.';
        end
        ...
        function obj = calcfunc(obj)
            obj.helpStruct.calcfunc = [...
                'Name of the custom function used for calculating the time-series. See the scripts myCalcFunc and myFetchDataAndCalcFunc' ...
                'for an example how to setup a manual calculator using a custom function.'];
        end

        function obj = gamma(obj)
            obj.helpStruct.gamma = [...
                'Hyperparameter on prior variance of the coefficients of the state equations. On the form V(i,j) = ',...
                'gamma./(ceil(j/options.nFactors).^2). Where V is a matrix with size option.nFactors x option.nLags',...
                '*option.nFactors. Default value is 0.1.'];
        end

        function obj = gammaxx(obj)
            obj.helpStruct.gammaxx = [...
                'A (C + N*L) x (C + N*L) double with the nonstandardized sample moments of the right-hand variables ',...
                'of the VAR. N is the number of endogenous variables, L is the number of lags of the VAR, and C = 1 ',...
                'if the model contain a constant term, otherwise is C = 0. Can be found from a nb_model_generic model ',...
                'using the method nb_model_generic.getDSGEVARPriorMoments'];
        end

        function obj = gammaxy(obj)
            obj.helpStruct.gammaxy = [...
                'A (C + N*L) x N double with the nonstandardized sample moments between the right-hand variables ',...
                'and the left-hand variables of the VAR. N is the number of endogenous variables, L is the number of ',...
                'lags of the VAR, and C = 1 if the model contain a constant term, otherwise is C = 0.Can be found ',...
                'from a nb_model_generic model using the method nb_model_generic.getDSGEVARPriorMoments'];
        end

        function obj = gammayy(obj)
            obj.helpStruct.gammayy = [...
                'A N x N double with the nonstandardized sample moments of the left-hand variables of the VAR. N is the ',...
                'number of endogenous variables. Can be found from a nb_model_generic model using the method ',...
        ...
        function obj = h(obj)
            obj.helpStruct.h = ...
                'Matrix that specifies the cointegration relations. For more see nb_var.applyLongRunPriors.';
        end
        ...
        function obj = lambda(obj)
            ...
            elseif strcmpi(obj.nameOfPOrC,'nb_laplacePrior') || strcmpi(obj.nameOfPOrC,'nb_var.priorTemplate')
                if strcmpi(obj.typeName,'glp')
                    obj.helpStruct.lambda = ['Hyperparameter that controls the overall ',...
                        'tightness of this prior. Defualt is 0.2. As a scalar double.'];
                elseif strcmpi(obj.typeName,'dsge')
                    obj.helpStruct.lambda = 'DSGE-VAR weight. As a scalar double.';
                else
                    obj.helpStruct.lambda = ['The "regularization" coefficient of the bayesian LASSO, i.e. ',...
                        'lambda of the prior Laplace(0,2*sigma(i)^2/lambda). Setting lambda amount to ',...
                        'setting the lambda of the lagrangian of the LASSO problem; min (y - X*beta)^2 + ',...
                        'lambda*(sum(abs(beta)) - 1/t). Lambda must be scalar or a vector with same ',...
                        'length as the number of dependent variables of the model.'];
                end
            else
        ...
        function obj = lambdahyperprior(obj)
            obj.helpStruct.lambdahyperprior = ...
                ['Hyperprior for the ''lambda'' prior. A function handle, that takes one input, and that ',...
                'input is a scalar double with the value of ''lambda''.'];
        end

        function obj = lambda0varscale(obj)
            obj.helpStruct.lambda0varscale = ...
                ['Scale factor on the variance of the prior on the initial value of the factor loadings. ',...
                 'Default is 1. N(0,lambdaVarScale*I).'];
            if any(strcmpi(obj.nameOfPOrC,{'nb_var.priorTemplate','nb_mfVar.priorTemplate'}))
                obj.helpStruct.lambda0varscale = [obj.helpStruct.lambda0varscale, ' Do not adjust!'];
            end
        end
        
        function obj = l_1m(obj)
            obj.helpStruct.l_1m = ...
                ['Starting value of: Decay factor for the measurement error variance of the monthly variables. A ',...
                'smaller value puts smaller weight on past observations and thus allows for faster parameter ',...
                'change. A value of 1 implies constant parameters. Default is 1.'];
            if any(strcmpi(obj.nameOfPOrC,{'nb_var.priorTemplate','nb_mfVar.priorTemplate'}))
                obj.helpStruct.l_1m = [obj.helpStruct.l_1m, ' Do not adjust!'];
            end
        end

        function obj = l_1q(obj)
            obj.helpStruct.l_1q = ...
                ['Starting value of: Decay factor for the measurement error variance of the quarterly variables. A ',...
                'smaller value puts smaller weight on past observations and thus allows for faster parameter change. ',...
                'A value of 1 implies constant parameters. Default is 1.'];
            if any(strcmpi(obj.nameOfPOrC,{'nb_var.priorTemplate','nb_mfVar.priorTemplate'}))
                obj.helpStruct.l_1q = [obj.helpStruct.l_1q, ' Do not adjust!'];
            end
        end

        function obj = l_2(obj)
            obj.helpStruct.l_2 = ...
                ['Starting value of: Decay factor for the factor error variance. A smaller value puts smaller weight on ',...
                 'past observations and thus allows for faster parameter change. A value of 1 implies constant parameters. ',...
                 'Default is 1.'];
        end

        function obj = l_3(obj)
            obj.helpStruct.l_3 = ...
                ['Starting value of: Decay factor for the loadings error variance. A smaller value puts smaller weight ',...
                'on past observations and thus allows for faster parameter change. A value of 1 implies constant ',...
                'parameters. Default is 1.'];
            if any(strcmpi(obj.nameOfPOrC,{'nb_var.priorTemplate','nb_mfVar.priorTemplate'}))
                obj.helpStruct.l_3 = [obj.helpStruct.l_3, ' Do not adjust!'];
            end
        end

        function obj = l_4(obj)
            obj.helpStruct.l_4 = ...
                ['Starting value of: Decay factor for the factor VAR parameters'' error variance. A smaller value ',...
                 'puts smaller weight on past observations and thus allows for faster parameter change. A value ',...
                 'of 1 implies constant parameters. Default is 1.'];
        end

        function obj = l_1_endo_update(obj)
            obj.helpStruct.l_1_endo_update = ...
                ['Controls the endogenous forgetting factors. 1: l_1m and l_1q are time-varying/endogenous ',...
                 '0: l_1m and l_1q are constant/static. Default is 0.'];
            if any(strcmpi(obj.nameOfPOrC,{'nb_var.priorTemplate','nb_mfVar.priorTemplate'}))
                obj.helpStruct.l_1_endo_update = [obj.helpStruct.l_1_endo_update, ' Do not adjust!'];
            end
        end

        function obj = l_2_endo_update(obj)
            obj.helpStruct.l_2_endo_update = ...
                ['Controls the endogenous forgetting factors. 1: l_2 is time-varying/endogenous ',...
                 '0: l_2 is constant/static. Default is 0.'];
        end

        function obj = l_3_endo_update(obj)
            obj.helpStruct.l_3_endo_update = ...
                ['Controls the endogenous forgetting factors. 1: l_3 is time-varying/endogenous ',...
                 '0: l_3 is constant/static. Default is 0.'];
            if any(strcmpi(obj.nameOfPOrC,{'nb_var.priorTemplate','nb_mfVar.priorTemplate'}))
                obj.helpStruct.l_3_endo_update = [obj.helpStruct.l_3_endo_update, ' Do not adjust!'];
            end
        end

        function obj = l_4_endo_update(obj)
            obj.helpStruct.l_4_endo_update = ...
                ['Controls the endogenous forgetting factors. 1: l_4 is time-varying/endogenous ',...
                 '0: l_4 is constant/static. Default is 0.'];
        end
        ...
        function obj = logtransformation(obj)
            obj.helpStruct.logtransformation = ...
                ['Set to true to use the log transformation -log((MAX-VALUE)./(VALUE-MIN)) to the ',...
                 'parameters to optimize over. May be important if the optimizer chosen do not ',...
                 'support lower and upper bounds! Used when the option ''hyperLearning'' of the ',...
                 'model is set to true.'];
        end

        function obj = lr(obj)
            obj.helpStruct.lr = ...
                ['Apply the prior for the long run as in Giannone et. al (2014). See also ',...
        ...
        function obj = maxtries(obj)
            
            obj.helpStruct.maxtries = ...
                'Maximum tries when trying to find a stationary draw of the parameters of the model. Default is 100.';
        ...
        function obj = method(obj)
            ...
            elseif strcmpi(obj.nameOfPOrC,'nb_model_selection_group')
                obj.helpStruct.method = ...
                    'See the corresponding input to the nb_model_generic.forecast method.';
            elseif strcmpi(obj.nameOfPOrC,'nb_synthesizer')
                obj.helpStruct.method = ...
                    ['Method to use for creating synthetic series. 1: Cross validate models, estimate on full sample and weigh them ',...
                    'together. 2: Estimate models on full sample and estimate a factor using PCA'];
            elseif nb_contains(obj.nameOfPOrC,'priorTemplate')
                obj.helpStruct.method = ...
                    ['Sets the method to use to draw from the posterior. Either ''default'' (default) or any ',...
                     'other string. ''default'' uses (a fixed) covariance matrix of the shocks (i.e. the prior). ',...
                     'Otherwise it also samples from the posterior distribution of the covariance matrix, as ',...
                     'in the same way as in the case of the independent normal wishart (inwishart) prior. The ',...
                     '''burn'', ''thin'' and ''S_scale'' options only applies to this last case.'];
            else
        ...
        function obj = mixing(obj)
            if strcmpi(obj.nameOfPOrC,'nb_mfvar.priorTemplate')
                obj.helpStruct.mixing = ...
                    ['Set to ''low'' to use the low frequency observations when forming the prior of the ',...
                     'covariance matrix of the residuals. Only in use if you have declared variables as ',...
                     'mixing. Default is to use the high frequency variable (''high'').'];
            else
                obj.helpStruct.mixing = ...
        ...
        function obj = models(obj)
            obj.helpStruct.models = ...
                'A struct array of the model templates to combine.';
        end
        ...
        function obj = modelweights(obj)
            obj.helpStruct.modelweights = ...
                ['A 1 x nmodels vector with weights used to combine the nmodels best models if method 1 is used. ',...
                 'Defaults to weights that are proportional to the scoreCrit.'];
        end

        function obj = mu(obj)
            obj.helpStruct.mu = ...
                ['Shrinkage parameter of the sum-of-coefficients prior. As mu goes to 0 implies the presence ',...
                 'of a unit root in each equation and rules out cointegration.'];
        end

        function obj = muhyperprior(obj)
            
            obj.helpStruct.muhyperprior = ...
                ['Hyperprior for the ''mu'' prior. A function handle, that takes one input, and that ',...
                'input is a scalar double with the value of ''mu''.'];
        ...
        function obj = nlags(obj)
            ...
            elseif strcmpi(obj.nameOfPOrC,'nb_ljungBoxTestStatistic')
                obj.helpStruct.nlags = ...
                    ['Number of lags of the autocorrelations to sum. Default is ceil(T/4), where T ',...
                     'is the sample size. For more on this see nb_ljungBoxTest.'];
            else
        ...
        function obj = nmodels(obj)
            obj.helpStruct.nmodels = ...
                'Sets the number of models to combine.';
        end
        
        function obj = nonstationary(obj)
            if strcmpi(obj.nameOfPOrC,'nb_var.priorTemplate')
                obj.helpStruct.nonstationary = ...
                    'Set to true to use some tricks to be able to handle problems related to models in levels.';
            else
                obj.helpStruct.nonstationary = ...
        ...
        function obj = optimset(obj)
            ...
            switch obj.nameOfPOrC
                ...
                case 'nb_var'
                    obj.helpStruct.optimset = ...
                        ['Sets the optimization options used for estimation. For LASSO estimation see the nb_lasso.optimset ',...
                         'or the nb_getDefaultOptimset function.  ''estim_method'' must be set to ''lasso''. For ',...
                         '''estim_method'' set to ''ml'' use the nb_getDefaultOptimset The optimizer can in this case ',...
                         'be set by the ''optimizer'' option. If [] is given, default optimization options are applied.']; 
                case 'nb_mlEstimator'
                    obj.helpStruct.optimset = ...
                        ['Sets the optimization options used for estimation. See nb_getDefaultOptimset for default values ',...
                        'given a optimizer. The optimizer can in this case be set by the ''optimizer'' option.']; 
                otherwise
                    obj.helpStruct.optimset = ...
        ...
        function obj = optparam(obj)
            obj.helpStruct.optparam = ...
                ['A cellstr with the parameters to optimize over or empty. If empty it ',...
                'tries to optimize over all hyper parameters of the selected prior. Used ',...
                'when the option ''hyperLearning'' of the model is set to true.'];
        end
        ...
        function obj = outvariables(obj)
            ...
            elseif strcmp(obj.nameOfPOrC,'nb_manualCalcEstimator')
                obj.helpStruct.outvariables = [...
                    'The variable names of the resulting calculated time-series. ',...
                    'Must be a cellstr with length M. For more see the calcFunc option.'];
            else
                obj.helpStruct.outvariables = ...
        ...
        function obj = periodsmax(obj)
            
            obj.helpStruct.periodsmax = ...
                ['Number of periods we allow for estimation of s_t of the stochastic-volatility-dummy ',...
                 'prior. Default is 3.'];
            
        end    

        function obj = phi(obj)
            
            obj.helpStruct.phi = ...
                'Shrinkage parameter for the long run prior. For more see nb_var.applyLongRunPriors.';
            
        end

        function obj = phihyperprior(obj)
            
            obj.helpStruct.phihyperprior = ...
                ['Hyperprior for the ''phi'' prior. A function handle, that takes one input, and that ',...
                'input is the vector of doubles with the values of ''phi'' parameters.'];
        ...
        function obj = polylags(obj)
            ...
            obj.helpStruct.polylags = [...
                ...
                'number of regressors to construct from the lags. As a scalar double or a cell ',...
                'with the same length as the number of exogenous variables. No elements of ',...
                '''polyLags'' input can be set to a number greater than 3.'];
        ...
        function obj = q0varscale(obj)
            obj.helpStruct.q0varscale = ...
                ['Scale factor on the mean of the prior on the initial value of the state equation covariance ',...
                 'matrix. Default is 0.1. Dogmatic prior set to Q0VarScale*I.'];
        end
        ...
        function obj = r_scale(obj)
            obj.helpStruct.r_scale = ...
                ['Inverse prior scale of R, i.e. the covariance matrix of the measurement errors. Default is ',...
                 '[], i.e. no measurement error. To apply it to specific variables use a cell array. In the ',...
                 'first column you must provide the name of dependent variable. E.g. ''Var1''. In the second ',...
                 'column you give the prior value, as a scalar double. Full example {''Var1'',inf;''Var2'',10}. ',...
                 'Here you allow for no measurement error in ''Var1'', and some measurement error in ''Var2'' .',...
                 'All variables not provided when a cell is used, are set to inf. If instead set to a scalar ',...
                 'the same scaling parameter is used for all variables. The posterior is constructed by ',...
                 'dividing the variance of each series by the scaling parameters.'];
        end
        ...
        function obj = removeperiods(obj)
            obj.helpStruct.removeperiods = ...
                ['Periods to remove from all models before estimation as a n x 2 cell where the first ',...
                'column contain the start date of the period(s) to remove and the second column contain ',...
                'the end date of the period(s) to remove. Dates may be chars or nb_date objects.'];
        end
        ...
        function obj = restrictconstant(obj)
            obj.helpStruct.restrictconstant = ...
                ['Restrict estimate of constant term to OLS estimate, when estimating the model with LASSO or RIDGE. ',...
        ...
        function obj = rho(obj)
            
            obj.helpStruct.rho = ...
                ['Decay parameter, used for s_t after reaching periodsMax. Used by the stochastic-volatility-dummy ',...
                 'prior. Default is 0.5.'];
            
        end

        function obj = rhohyperprior(obj)
            
            obj.helpStruct.rhohyperprior = ...
                ['Hyperprior for the ''rho'' prior. A function handle, that takes one input, and that ',...
                'input is a scalar double with the value of ''rho''.'];

        end
        
        function obj = regularization(obj)
            obj.helpStruct.regularization = ...
                ['Which value to use for L1 or L2 regularization on the parameters when the model is estimated ',...
                 'with LASSO (t input to nb_lasso) or RIDGE (k input to nb_ridge) respectively, i.e. for RIDGE we ',...
                 'do not set the L2 regularization directly, but we instead set the lagrange mulitplier. Default ',...
                 'is [], i.e. the default parameter of the functions nb_lasso or nb_ridge are used. For both ',...
                 'you can set the regularization individually for each equation by providing a double vector ',...
                 'with the same length as the number of estimated equations of the model. Examples; 1 or [1,0.5,1]. ',...
                 'Caution: For LASSO you set the L1 regularization of the full problem, even if the constant is ',...
                 'restricted to the OLS estimate or not, while for RIDGE you set the lagrange mulitplier for the ',...
                 'restricted problem only (if it is restricted). Set the ''restrictConstant'' option to false ',...
                 'to stop restricting the constant term.'];
            
        end

        function obj = regularizationperc(obj)
            obj.helpStruct.regularizationperc = ...
                ['Instead of setting the absolute value for the L1 or L2 regularization on the parameters when ',...
                 'the model is estimated with LASSO or RIDGE. You can set it to be a percentage of the value ',...
                 'that makes the problem unrestricted. For more see the ''tPerc'' input to the nb_lasso function ',...
                 'or the ''cPerc'' input to the nb_ridge function. If you set this option, the ''regularization'' ',...
                 'option is ignored. Must be set to a scalar double or a 1 x nEq double. In the last case you ',...
                 'can set different values for each equation of the model Caution: In this case the ',...
                 'regularization coefficient may be different across equations! Finally, if the ''restrictConstant'' ',...
                 'option is set to true (default), the constant term is excluded from the calculation of the ',...
                 'percentage!'];
        ...
        function obj = s_scale(obj)
            if nb_contains(obj.typeName,'nwishart')
                obj.helpStruct.s_scale = ...
                    'Prior scale of prior on sigma. Default is 1. ';
            else
                obj.helpStruct.s_scale = ...
                    'Prior scale of prior on sigma. Default is 1, i.e. OLS. ';
            end
        end

        function obj = s_scalehyperprior(obj)
            obj.helpStruct.s_scalehyperprior = ...
                ['Hyperprior for the ''S_scale'' prior. A function handle, that takes one input, and that ',...
                'input is a scalar double with the value of ''S_scale''.'];
        end
        ...
        function obj = sc(obj)
            obj.helpStruct.sc = ...
                'Apply the sum-of-coefficients prior by Doan, Litterman, and Sims (1984). true or false';
        end
        
        function obj = scorecrit(obj)
            obj.helpStruct.scorecrit = ...
                ['Evaluation criterion to apply to out-of-sample prediction errors. Possible options are: ',...
                nb_synthesizer.availableScoreCrit '. Default is RMSE.'];
        end
        ...
        function obj = store2second(obj)
            if strcmpi(obj.nameOfPOrC,'nb_calculate_vintages') 
                 obj.helpStruct.store2second = ...
                    ['Set the second database to store the calculated series to. Must be set to an object which ',...
                     'is of a subclass of the nb_store2Database class. Use the nb_store2Database.getSubClasses() ',...
                     'method to get the available subclasses.'];
            else
                obj.helpStruct.store2second = ...
                    ['Set the second database to store the forecasts to. Must be set to an object which is of a ',...
                     'subclass of the nb_writeFcst2Database class. Use the nb_writeFcst2Database.getSubClasses() ',...
                     'method to get the available subclasses.'];
            end
        end
        ...
        function obj = svd(obj)
            obj.helpStruct.svd = ...
                ['Apply the stochastic-volatility-dummy prior "How to estimate a vector autoregression ',...
                 'after March 2020" by Lenza and Primiceri (2020). true or false'];
        end
        ...
        function obj = thin(obj)
            obj.helpStruct.thin = ['Every k draws are kept when running gibbs sampler. This options sets ',...
                'k. Default is 2. Increase this option to prevent autocorrelated draws.'];
            if nb_contains(obj.nameOfPOrC,'priorTemplate')
                obj.helpStruct.thin = [obj.helpStruct.thin,...
                    ' See nb_model_generic.checkPosteriors to test for autocorrelated draws.'];
            end
        end
        ...
        function obj = type(obj)
            if nb_contains(obj.nameOfPOrC,'priorTemplate')
                obj.helpStruct.type = ...
                    'The type of prior the options are set up for.';
            elseif strcmpi(obj.nameOfPOrC,'nb_ljungBoxTestStatistic')
                obj.helpStruct.type = ...
                    '''multivariate'' (default) or ''univariate''. For more see the function nb_ljungBoxTest.';
            else
                obj.helpStruct.type = ...
        ...
        function obj = varofinterest(obj)
            ...
            elseif strcmpi(obj.nameOfPOrC,'nb_synthesizer')
                obj.helpStruct.varofinterest = ... 
                    'The variable for which a synthetic series should be created.';
            else
                obj.helpStruct.varofinterest = ...
        ...
        function obj = vc(obj)
            obj.helpStruct.vc = ...
                'Hyperparamter on exogenous variables. Default is 1e7.';
        end

        function obj = vchyperprior(obj)
            obj.helpStruct.vchyperprior = ...
                ['Hyperprior for the ''Vc'' prior. A function handle, that takes one input, and that ',...
                'input is a scalar double with the value of ''Vc''.'];
        end

        function obj = v_scale(obj)
            obj.helpStruct.v_scale = ...
                'Scale of the prior of the variance of the coefficients. Default is 10. ';
        end

        function obj = v_scalehyperprior(obj)
            obj.helpStruct.v_scalehyperprior = ...
                ['Hyperprior for the ''V_scale'' prior. A function handle, that takes one input, and that ',...
                'input is a scalar double with the value of ''V_scale''.'];
        end

        function obj = v0varscale(obj)
            obj.helpStruct.v0varscale = ...
                ['Scale factor on the mean of the prior on the initial value of the measurement equation ',...
                 'covariance matrix. Default is 0.1. Dogmatic prior set to V0VarScale*I.'];
            if any(strcmpi(obj.nameOfPOrC,{'nb_var.priorTemplate','nb_mfVar.priorTemplate'}))
                obj.helpStruct.v0varscale = [obj.helpStruct.v0varscale, ' Do not adjust!'];
            end
        end
        ...
        function obj = notDocumented(obj,option) 
            if ~strcmpi(obj.typeName,{'class','package'})
                if isempty(obj.typeName)
                    obj.helpStruct.(lower(option)) = ['No help found for the option ''' option, ''' of the struct returned by the function ' obj.nameOfPOrC ];
                else
                    obj.helpStruct.(lower(option)) = ['No help found for the option ''' option, ''' of the ''' obj.typeName ''' prior.'];
                end
            else
