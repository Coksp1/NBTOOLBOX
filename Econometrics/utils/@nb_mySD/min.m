function obj = min(obj,another,flip)
% Syntax:
%
...
% Copyright (c) 2023, Kenneth Sæterhagen Paulsen
    ...
    if isa(another,'nb_mySD') && isa(obj,'nb_mySD')
        ...
        for ii = 1:nNew
            ...
            if indB1(ii) && indB2(ii)
                ...
                elseif strcmp(obj.derivatives{iter1},another.derivatives{iter2})  
                    ...
                    newDerivs{ii} = strcat('nb_minDeriv(', obj.values, ',', another.values, ',', obj.derivatives{iter1},',', another.derivatives{iter2}, ')');
                end
                iter1 = iter1 + 1;
                iter2 = iter2 + 1;
            ...
            elseif indB1(ii)
                ...
                if test1 == 0
                    ...
                    newDerivs{ii} = strcat('nb_minDeriv(', obj.values, ',', another.values, ',', obj.derivatives{iter1},',0)');
                end
                iter1 = iter1 + 1;  
            elseif indB2(ii)
                test2 = str2double(another.derivatives{iter2});
                if test2 == 0
                    ...
                    newDerivs{ii} = strcat('nb_minDeriv(', obj.values, ',', another.values, ',0,', another.derivatives{iter2}, ')');
                end
                iter2 = iter2 + 1;              
            end
        ...
        obj.bases       = basesNew;
        obj.values      = ['min(' obj.values ',' another.values ')'];
    ...
    elseif isa(obj,'nb_mySD') && (nb_isScalarNumber(another) || nb_isOneLineChar(another) || isa(another,'nb_param'))
        ...
        for ii = 1:length(obj.derivatives)
            ...
            if test == 0
                ...
                obj.derivatives{ii} = strcat('nb_minDeriv(', obj.values, ',', anotherStr, ',', obj.derivatives{ii},',0)');
            end
        end
        obj.values = ['min(' obj.values ',' anotherStr ')'];
    ...
    elseif isa(another,'nb_mySD') && (nb_isScalarNumber(obj) || nb_isOneLineChar(obj) || isa(obj,'nb_param'))
        ...
        for ii = 1:length(another.derivatives)
            ...
            if test == 0
                ...
                another.derivatives{ii} = strcat('nb_minDeriv(', objStr, ',', another.values, ',0,', another.derivatives{ii}, ')');
            end
        end
        another.values = ['min(' another.values ',' objStr ')'];
        obj            = another;
    else
