...
% 
% [rp, ci, ai] = gaimc.sparse_to_csr(A) returns the row pointer (rp),  
% column index (ci) and value index (ai) arrays of a compressed sparse 
% representation of the matrix A.
%
% [rp ci ai] = gaimc.sparse_to_csr(i,j,v,n) returns a csr representation 
% of the index sets i,j,v with n rows.
%
% Example:
...
% 2009-05-15: Fixed triplet input
    ...
    if nargin>1
        ...
        if ~isscalar(n)
            error('gaimc:invalidInput',['the 4th input to sparse_to_csr with triple input was not a scalar']); 
        end
        if nargin < 5, ncol = max(nzj); 
        elseif ~isscalar(ncol)
            error('gaimc:invalidInput',['the 5th input to sparse_to_csr with triple input was not a scalar']); 
        end
    else
        n    = size(A,1); 
        nz   = nnz(A); 
