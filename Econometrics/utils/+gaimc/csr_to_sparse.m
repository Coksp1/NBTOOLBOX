...
%
% A = gaimc.csr_to_sparse(rp,ci,ai) returns the sparse matrix represented 
% by the compressed sparse row representation rp, ci, and ai.  The number 
% of columns of the output sparse matrix is max(max(ci),nrows). See the 
% call below.
%
% A = gaimc.csr_to_sparse(rp,ci,ai,ncol) While we can infer the number of 
% rows in the matrix from this expression, you may want a different number 
% of. 
%
% [nzi,nzj,nzv] = gaimc.csr_to_sparse(...) returns the arrays that feed 
% the sparse call in matlab.  You can use this to avoid the sparse call 
