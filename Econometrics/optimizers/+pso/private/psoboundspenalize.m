function state = psoboundspenalize(state,Aineq,bineq,Aeq,beq,LB,UB,nonlcon,options)
% Penalty-based constraint enforcement method.
    ...
    if ~strcmpi(options.UseParallel,'always') % Default option
        for i = 1:n
            ...
            [c,ceq] = nonlconwrapper(nonlcon,Aineq,bineq,Aeq,beq,LB,UB,tol,x(i,:)) ;
...
function [c,ceq] = nonlconwrapper(nonlcon,Aineq,bineq,Aeq,beq,LB,UB,TolCon,x)
