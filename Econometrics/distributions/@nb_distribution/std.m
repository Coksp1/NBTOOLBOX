    ...
    for jj = 1:nobj1
        ...
        for ii = 1:nobj
            ...
            if ~isempty(lb{jj,ii}) || ~isempty(ub{jj,ii}) % Truncated distribution
                ...
                if isempty(ms{jj,ii})
                    x(jj,ii) = nb_distribution.truncated_std(obj(jj,ii).type,obj(jj,ii).parameters,lb{jj,ii},ub{jj,ii});
                else
                    x(jj,ii) = nb_distribution.meanshift_std(obj(jj,ii).type,obj(jj,ii).parameters,lb{jj,ii},ub{jj,ii},ms{jj,ii});
                end
                ...
                func     = str2func(['nb_distribution.' obj(jj,ii).type '_std']);
