...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
   ...
   properties (SetAccess=protected)
       ...
       %                 degrees of freedom.
       %
       % - 'skt'       : The paramters property must be set to a 1x4 cell.
       %                 {location,scale,shape,dof}. Default is 
       %                 {0,1,0,inf}, i.e. same as the normal distribution.
       %                 dof is the degree of freedom, and must be set to 
       %                 a 1x1 double greater than 0. shape must be in
       %                 (-1,1). scale > 0. No restrictions on location.
       %
       ...
       %
       % - 'skt'       : Azzalini skewed t-distribution. Azzalini, A. and 
       %                 Capitanio,A. (2003). Distributions generated by 
       %                 perturbation of symmetry with emphasis on a 
       %                 multivariate skew-t distribution. 
       %                 J.Roy.Statist.Soc. B 65, 367-389. May not be
   ...
   methods(Static=true)
       ...
       varargout = skt_cdf(varargin)
       varargout = skt_domain(varargin)
       varargout = skt_icdf(varargin)
       varargout = skt_kurtosis(varargin);
       varargout = skt_mean(varargin);
       varargout = skt_median(varargin);
       varargout = skt_mode(varargin);
       varargout = skt_pdf(varargin)
       varargout = skt_rand(varargin)
       varargout = skt_skewness(varargin);
       varargout = skt_std(varargin);
       varargout = skt_variance(varargin);
