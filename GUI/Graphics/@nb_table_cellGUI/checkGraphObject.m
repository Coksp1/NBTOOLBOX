...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    if ~isa(propertyValue,'nb_table_cell')
          
        if isa(propertyValue,'nb_graph_data') || isa(propertyValue,'nb_table_data')
            nb_errorWindow('It is not possible to load up a dimensionless graph/table to a cell table window.');
            return
        elseif isa(propertyValue,'nb_graph_ts') || isa(propertyValue,'nb_table_ts')
            nb_errorWindow('It is not possible to load up a time-series graph/table to a cell table window.');
            return
        elseif isa(propertyValue,'nb_graph_cs') || isa(propertyValue,'nb_table_cs')
            nb_errorWindow('It is not possible to load up a cross-sectional graph/table to a cell table window.');    
            return    
        else
