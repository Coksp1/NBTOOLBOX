...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    if ~isa(propertyValue,'nb_table_ts')
          
        if isa(propertyValue,'nb_graph_data') || isa(propertyValue,'nb_table_data')
            nb_errorWindow('It is not possible to load up a dimensionless graph/table to a time-series table window.');
            return
        elseif isa(propertyValue,'nb_graph_cs') || isa(propertyValue,'nb_table_cs')
            nb_errorWindow('It is not possible to load up a cross-sectional graph/table to a time-series table window.');
            return
        elseif isa(propertyValue,'nb_graph_ts') 
            nb_errorWindow('It is not possible to load up a time-series graph to a time-series table window.');    
            return
        elseif isa(propertyValue,'nb_table_cell') 
            nb_errorWindow('It is not possible to load up a cell table to a time-series table window.');    
            return     
        else
            ...
            elseif isa(propertyValue.plotter,'nb_graph_cs') || isa(propertyValue,'nb_table_cs')
                nb_errorWindow('It is not possible to load up a cross-sectional graph/table to a time-series table window.');
                return
            elseif isa(propertyValue.plotter,'nb_graph_cs') 
                nb_errorWindow('It is not possible to load up a time-series graph to a time-series table window.');    
                return 
