...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    if ~isa(propertyValue,'nb_graph_ts')
          
        if isa(propertyValue,'nb_graph_data') || isa(propertyValue,'nb_table_data')
            nb_errorWindow('It is not possible to load up a dimensionless graph/table to a time-series graph window.');
            return
        elseif isa(propertyValue,'nb_graph_cs') || isa(propertyValue,'nb_table_cs')
            nb_errorWindow('It is not possible to load up a cross-sectional graph/table to a time-series graph window.');
            return
        elseif isa(propertyValue,'nb_table_ts') 
            nb_errorWindow('It is not possible to load up a time-series table to a time-series graph window.');    
            return
        elseif isa(propertyValue,'nb_table_cell') 
            nb_errorWindow('It is not possible to load up a cell table to a time-series graph window.');    
            return    
        else
            if isa(propertyValue.plotter,'nb_graph_data') || isa(propertyValue,'nb_table_data')
                nb_errorWindow('It is not possible to load up a dimensionless graph/table to a time-series graph window.');
                return
            elseif isa(propertyValue.plotter,'nb_graph_cs') || isa(propertyValue,'nb_table_cs')
                nb_errorWindow('It is not possible to load up a cross-sectional graph/table to a time-series graph window.');
                return
    ...
    for ii = 1:size(newPlotter,2)
        ...
        try
            ...
            if strcmpi(ME.identifier,'nb_graph:LocalVariableError')
                ...
                eMessage = [eMessage, ' is/are deleted or cannot be interpreted. Graph cannot be produced.'];  %#ok<AGROW>
