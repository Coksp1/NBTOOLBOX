...
% Copyright (c) 2021, Kenneth Sæterhagen Paulsen
    ...
    if ~isa(propertyValue,'nb_graph_data')
          
        if isa(propertyValue,'nb_graph_cs') || isa(propertyValue,'nb_table_cs')
            nb_errorWindow('It is not possible to load up a cross-sectional graph/table to a dimensionless graph window.');
            return
        elseif isa(propertyValue,'nb_graph_ts') || isa(propertyValue,'nb_table_ts')
            nb_errorWindow('It is not possible to load up a time-series graph/table to a dimensionless graph window.');
            return
        elseif isa(propertyValue,'nb_table_data') 
            nb_errorWindow('It is not possible to load up a dimensionless table to a dimensionless graph window.');    
            return
        elseif isa(propertyValue,'nb_table_cell') 
            nb_errorWindow('It is not possible to load up a cell table to a dimensionless graph window.');    
            return    
        else
            if isa(propertyValue.plotter,'nb_graph_cs') || isa(propertyValue,'nb_table_cs')
                nb_errorWindow('It is not possible to load up a cross-sectional graph/table to a dimensionless graph window.');
                return
            elseif isa(propertyValue.plotter,'nb_graph_ts') || isa(propertyValue,'nb_table_ts')
                nb_errorWindow('It is not possible to load up a time-series graph/table to a dimensionless graph window.');
                return
            elseif isa(propertyValue.plotter,'nb_table_data') 
                nb_errorWindow('It is not possible to load up a dimensionless table to a dimensionless graph window.');    
                return  
        ...
        try
            ...
            if strcmpi(ME.identifier,'nb_graph:LocalVariableError')
                ...
                for jj = 2:length(lVars)
                    eMessage = [eMessage,', ' ,lVars{jj}];   %#ok<AGROW>
                end
