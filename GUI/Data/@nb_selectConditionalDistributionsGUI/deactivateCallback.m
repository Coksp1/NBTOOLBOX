function deactivateCallback(gui,~,~)
% Syntax:
%
% deactivateCallback(gui,hObject,event)
%
% Description:
%
% Part of DAG.
...
function finish(~,~,gui,selList)
    ...
    for ii = 1:size(gui.distributions,1)
        for jj = ind'
            gui.distributions(ii,jj) = nb_distribution('type','constant','parameters',{mean(gui.distributions(ii,jj))});
        end
    end
